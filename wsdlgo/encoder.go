...
const fileHeader = "// Code generated by wsdlonthego. DO NOT EDIT."
	...
	for _, fn := range ge.funcnames {
		...
		if !ok {
			...
			for i, p := range outParams {
				ret[i] = ge.wsdlonthegoDefault(p.dataType)
			}
...
func (ge *goEncoder) writeSOAPFunc(w io.Writer, d *wsdl.Definitions, op *wsdl.Operation, in, out []*parameter) bool {
	...
	ge.needsExtPkg["github.com/sbottolo/wsdlonthego/soap"] = true
	...
	for index, name := range out {
		...
		if !strings.HasPrefix(name.dataType, "*") {
			...
			retDefaults[index] = ge.wsdlonthegoDefault(name.dataType)
		}
	}
...
func (ge *goEncoder) genParams(m *wsdl.Message, needsTag bool) []*parameter {
	...
	for i, param := range m.Parts {
		...
		switch {
		case param.Type != "":
			t = ge.wsdlonthegoType(param.Type)
		...
		case param.Element != "":
			...
			if el, ok := ge.elements[elName]; ok {
				t = ge.wsdlonthegoType(trimns(el.Type))
			} else {
				t = ge.wsdlonthegoType(param.Element)
			}
...
func (ge *goEncoder) wsdlonthegoType(t string) string {
...
func (ge *goEncoder) wsdlonthegoDefault(t string) string {
	v := trimns(t)
	if v != "" && v[0] == '*' {
		v = v[1:]
	}
...
func (ge *goEncoder) writeGoTypes(w io.Writer, d *wsdl.Definitions) error {
	...
	for _, name := range ge.sortedSimpleTypes() {
		...
		if st.Restriction != nil {
			...
			fmt.Fprintf(&b, "type %s %s\n\n", stname, ge.wsdlonthegoType(st.Restriction.Base))
...
func (ge *goEncoder) genAttributeField(w io.Writer, attr *wsdl.Attribute) {
	...
	typ := ge.wsdlonthegoType(attr.Type)
